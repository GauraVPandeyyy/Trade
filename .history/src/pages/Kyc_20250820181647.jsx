...
function Kyc() {
  ...
  const fileToBase64 = (file) => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => resolve(reader.result); // Send complete base64 string with prefix
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!aadharFront?.file || !aadharBack?.file || !panImage?.file) {
      ...
      const aadhaar_front_base64 = await fileToBase64(aadharFront.file);
      const aadhaar_back_base64 = await fileToBase64(aadharBack.file);
      const pan_front_base64 = await fileToBase64(panImage.file);

      // Prepare KYC data
      const kycData = {
        user_id: user.user_id,
        adhar_card: aadhar_card.replace(/\s/g, ''),  // Remove any spaces
        aadhaar_front: aadhaar_front_base64,
        aadhaar_back: aadhaar_back_base64,
        pancard: panNum.toUpperCase(),  // Convert to uppercase
        pan_front: pan_front_base64
      };

      console.log("Sending KYC data with images");
  ...
  return (
    ...
    className={`w-full px-4 py-3 border ${
        errors.adhar_card ? 'border-red-500' : 'border-gray-300'
