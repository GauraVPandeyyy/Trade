...
const DashboardCard = ({ icon: Icon, label, value, colorClass = "text-blue-600", trend, delay = 0 }) => {
  ...
  return (
    <div 
      className={`
        group relative bg-white rounded-2xl p-3 shadow-sm border border-gray-200 
    ...
    >
      ...
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-white to-gray-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      
      <div className="relative z-10 flex items-start justify-between">
        <div className="flex-1">
          <p className="text-xs font-medium text-gray-500 mb-3 tracking-wide uppercase">{label}</p>
          <div className="flex items-baseline space-x-2">
            <p className="text-l font-bold text-gray-900 tracking-tight">{value}</p>
            {trend && (
