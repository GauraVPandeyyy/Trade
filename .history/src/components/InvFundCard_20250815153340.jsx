...
function InvFundCard({ icon: Icon, product, capital, ROI, joinDate, endDate, badge, delay = 300 }) {
    ...
    useEffect(() => {
        const fetchInvFundData = async () => {
            try {
                const response = axios.get(`${API_BASE_URL}/investment-summary/${user.user_id}`);
            } catch (error) {
                console.error("Error fetching uinvestment-summary :", error);
                toast.error("Error fetching uinvestment-summary", error);
                throw error;
            }
        }
