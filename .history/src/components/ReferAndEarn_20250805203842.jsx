    ...
    <div className='w-full max-w-4xl mx-auto p-6 space-y-8'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
        <div className='bg-white dark:bg-gray-800 p-4 rounded-xl shadow'>
          <h3 className='text-gray-500 dark:text-gray-400 text-sm font-medium'>Total Invites</h3>
          <p className='text-2xl font-bold dark:text-white'>{promotionData.totalInvites || 0}</p>
        </div>
        <div className='bg-white dark:bg-gray-800 p-4 rounded-xl shadow'>
          <h3 className='text-gray-500 dark:text-gray-400 text-sm font-medium'>Active Users</h3>
          <p className='text-2xl font-bold dark:text-white'>{promotionData.activeUsers || 0}</p>
        </div>
        <div className='bg-white dark:bg-gray-800 p-4 rounded-xl shadow'>
          <h3 className='text-gray-500 dark:text-gray-400 text-sm font-medium'>Earnings</h3>
          <p className='text-2xl font-bold dark:text-white'>₹{promotionData.earnings?.toLocaleString() || '0'}</p>
        </div>
      </div>
      ...
      <div className='bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden'>
        ...
        {promotionData.history?.length > 0 ? (
          <div className='divide-y divide-gray-200 dark:divide-gray-700'>
            {promotionData.history.map((item, index) => (
              <div key={index} className='p-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors'>
                <div className='flex justify-between items-start'>
                  <div>
                    <h4 className='font-medium dark:text-white'>{item.user}</h4>
                    <p className='text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center'>
                      <ShoppingBag size={14} className='mr-1' />
                      {item.product} Products Purchased
                    </p>
                  </div>
