                ...
                <div key={name} className="w-full relative animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={formData[name]}
                      onChange={handleChange}
                      className={`w-full px-4 py-3 bg-gray-700 border ${errors[name] ? "border-red-500 shake-animation" : "border-gray-600"} rounded-lg text-white focus:outline-none focus:ring-2 ${errors[name] ? "focus:ring-red-500" : "focus:ring-blue-500"} transition-all pr-12`}
                      required
                    />
                    <button
                      type="button"
...
// Input name with inline error display
const InputField = ({
  label,
  name,
  type = "text",
